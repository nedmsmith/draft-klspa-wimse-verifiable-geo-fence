...
The SPIFFE JWT-SVID-based geographic boundary token is conveyed as a compact string in the `X-Workload-Geo-ID` HTTP header:

```
X-Workload-Geo-ID: <JWT-SVID>
```

Intermediaries and recipients MUST validate the JWT-SVID signature and claims according to the SPIFFE JWT-SVID standard.
